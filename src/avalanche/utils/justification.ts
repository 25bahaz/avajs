...
import { unpackRegisterL1ValidatorPayload, calculateValidationID, SolidityValidationPeriod } from './convertWarp';
...
function extractAddressedCall(messageBytes: Uint8Array): Uint8Array {
    try {
        ...
        const codecVersion = ((messageBytes[0] ?? 0) << 8) | (messageBytes[1] ?? 0);

        const messageLength = ((messageBytes[38] ?? 0) << 24) |
            ((messageBytes[39] ?? 0) << 16) |
            ((messageBytes[40] ?? 0) << 8) |
            (messageBytes[41] ?? 0);
...
function extractPayloadFromAddressedCall(addressedCall: Uint8Array): Uint8Array | null {
    try {
        ...
        const sourceAddrLen = ((addressedCall[6] ?? 0) << 24) | ((addressedCall[7] ?? 0) << 16) | ((addressedCall[8] ?? 0) << 8) | (addressedCall[9] ?? 0);
        ...
        // Read Payload Length
        const payloadLen = ((addressedCall[payloadLenPos] ?? 0) << 24) |
            ((addressedCall[payloadLenPos + 1] ?? 0) << 16) |
            ((addressedCall[payloadLenPos + 2] ?? 0) << 8) |
            (addressedCall[payloadLenPos + 3] ?? 0);
...
export async function GetRegistrationJustification(
    ...
    try {
        ...
        console.error(`Failed to decode provided validationIDHex '${validationIDHex}': ${e?.message ?? 'Unknown error'}`);
        return null;
    }
        ...
        while (!foundMatch && chunksSearched < MAX_CHUNKS) {
            ...
            if (warpLogs.length > 0) {
                for (const log of warpLogs.slice().reverse()) {
                    try {
                        const decodedArgs = log?.args as { message?: `0x${string}` } | undefined;
                        ...
                        const acTypeID = ((addressedCall[2] ?? 0) << 24) | ((addressedCall[3] ?? 0) << 16) | ((addressedCall[4] ?? 0) << 8) | (addressedCall[5] ?? 0);
                        ...
                        console.error(`Error processing log entry for tx ${log?.transactionHash ?? 'unknown'}:`, logProcessingError);
                    }
                }
            }
