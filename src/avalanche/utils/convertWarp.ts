...
// L1ValidatorRegistrationMessage format: codecID(2) + typeID(4, value=2) + validationID(32) + registered(1) = 39 bytes
function packL1ValidatorRegistrationPayload(validationID: Uint8Array, registered: boolean): Uint8Array {
    ...
    return concatenateUint8Arrays(
        encodeUint16(CODEC_ID),
        encodeUint32(L1_VALIDATOR_REGISTRATION_MESSAGE_TYPE_ID), // typeID = 2 for L1ValidatorRegistration
        validationID,
        new Uint8Array([registered ? 1 : 0]) // registered boolean
    );
}

/**
 * Packs a L1ValidatorRegistration message for sending (wrapped).
 * L1ValidatorRegistrationMessage is used to acknowledge validator registration.
 */
